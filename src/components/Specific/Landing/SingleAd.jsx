...
import ShareMetadata from "../../feutures/ShareMetaData";
...
const findOrCreateChat = async ({ adId, adCategoryId, adBuyerId, adSellerId, token, message, isOfferMessage }) => {
  try {
    if (isOfferMessage) {
      ...
      const { data } = await axios.get(`${BASE_URL}/api/find-one-chat/${adId}/${adCategoryId}/${adBuyerId}/${adSellerId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
...
function SingleAd() {
  ...
  const {
    ...
    isLoading,
    error,
  } = useQuery(["adData", adCategoryId, adId, token], fetchAdData, {
  ...
  const deleteFavoriteMutation = useMutation(
    async () => {
      ...
      const response = await axios.delete(`${BASE_URL}/api/ad-delete-favourite/${adId}/${adCategoryId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      return response.data;
    },
      ...
      <div className="my-8 font-Inter">
        <div className="w-[90%] mx-auto block">
          ...
          <div className="w-full md:w-[70%] mb-4 md:float-left md:pr-8">
            <div className="relative flex flex-col gap-4">
              ...
              <div className="relative flex items-center justify-center overflow-hidden rounded-lg h-[300px]">
                ...
                <button onClick={handlePrevClick} className="absolute left-4 z-10 p-2 rounded-full bg-black/30 hover:bg-black/50 transition-colors">
                ...
                <div className="absolute top-4 right-4 z-20 flex gap-3">
                  <Popover placement="bottom-end">
                    <PopoverTrigger>
                      <button className="p-2 rounded-full bg-black/50 hover:bg-black/70 transition-colors text-white" onClick={(e) => e.stopPropagation()}>
                        <Share2 size={20} />
                      </button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto bg-white rounded-lg shadow-lg p-2">
                      <PopoverBody>
                        <div className="flex gap-3 items-center">
                          <button onClick={handleWhatsAppShare} className="p-2 hover:bg-gray-100 rounded-full transition-colors" title="Share on WhatsApp">
                            <FaWhatsapp size={20} className="text-green-600" />
                          </button>

                          <button onClick={handleFacebookShare} className="p-2 hover:bg-gray-100 rounded-full transition-colors" title="Share on Facebook">
                  ...
                  {isLoggedIn && (
                    <button className="p-2 rounded-full transition-colors text-white" onClick={handleFavoriteClick}>
                      {isAdFavourite ? <BiSolidHeart className="w-5 h-5 text-red-500" /> : <BiHeart className="w-5 h-5 text-gray-600" />}
                    </button>
                  )}
                </div>

                <div className="w-full h-full px- bg-black flex justify-center items-center cursor-pointer" onClick={onOpen}>
                  <img src={currentImageUrl} alt={`Ad Image ${currentImageIndex + 1}`} className="max-h-full max-w-full object-contain" />
                  ...
                  <div className="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex gap-2">
                    {images.map((_, index) => (
                      <button
                        ...
                        ${currentImageIndex === index ? "bg-blue-500 scale-110" : "bg-gray-300 hover:bg-gray-400"}`}
                ...
                <button onClick={handleNextClick} className="absolute right-4 z-10 p-2 rounded-full bg-black/30 hover:bg-black/50 transition-colors">
                ...
                <div className="flex gap-4 overflow-x-auto py-2 px-2">
                  {images.map((image, index) => (
                    <button
                      ...
                      ${currentImageIndex === index ? "ring-2 ring-blue-500 ring-offset-2" : "hover:ring-2 hover:ring-gray-300 hover:ring-offset-2"}
                      rounded-lg overflow-hidden
                    `}
                    >
                      <img src={`${BASE_URL}${image.url}`} alt={`Thumbnail ${index + 1}`} className="w-16 h-16 object-cover" />
                    </button>
                  ))}
                </div>
              )}
          ...
          <div className="w-full md:hidden mb-4">
            <div className="grid grid-cols-12 gap-4">{isCarCategory ? <CarCar adData={adData} /> : <Rest adData={adData} />}</div>

            <div className="mt-8">
              <div className="font-semibold text-lg md:text-xl">Description</div>
              <p className="text-sm md:text-base text-gray-500 whitespace-pre-line">{formatDescription(adData.description)}</p>
            </div>
          </div>
          ...
          <div className="w-full md:w-[30%] flex flex-col gap-4 md:float-left">
            <div className="bg-white p-4 rounded-lg shadow-md">
              <div className="flex flex-col gap-4">
                <Button className="w-full bg-green-500 gap-2 text-white text-sm md:text-base py-6" onClick={handleMakeOffer}>
                  <FaCommentDollar /> Make Offer
                </Button>
                <Button className="w-full bg-green-600 gap-2 text-white text-sm md:text-base py-6" onClick={handleWhatsAppShare}>
                  <FaWhatsapp className="text-lg" /> Share on WhatsApp
                </Button>
                <Button className="w-full bg-gray-600 gap-2 text-white text-sm md:text-base py-6" onClick={handleCopyLink}>
            ...
            <div className="bg-white p-4 rounded-lg shadow-md">
              <div className="flex items-center justify-between cursor-pointer mb-4" onClick={handleSellerProfileClick}>
                <div className="flex items-center space-x-4">
                  <div className="w-12 h-12 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center">
                    {adData.adSeller?.profileImage?.url && <img src={`${BASE_URL}${adData.adSeller.profileImage.url}`} className="w-full h-full object-cover" alt="Seller" />}
                  </div>
              ...
              <Button className=" w-full bg-white border-cyan-600 border-2 gap-2 text-cyan-600 font-semibold text-sm md:text-base py-6 mb-4" onClick={handleChatWithSeller} isLoading={chatMutation.isLoading}>
                <FaComments /> Chat with Seller
              </Button>

              {/* Desktop Contact Link */}
              <div className="hidden md:flex justify-center">
                <button onClick={handleContactSeller} className="text-blue-500 font-semibold hover:underline focus:underline text-sm font-Inter transition-all duration-200">
                  Contact Seller
                </button>
              </div>
              ...
              <div className="md:hidden">
                <Button className="w-full bg-white border-green-600 border-2 gap-2 text-green-600 font-semibold text-sm md:text-base py-6 mb-4" onClick={handleCallSeller}>
                ...
                <div className="flex items-center gap-2 justify-center">
                  ...
                  {!showPhoneNumber && (
                    <button className="text-blue-500 underline text-14 font-Inter hover:text-blue-600" onClick={() => setShowPhoneNumber(true)}>
                      Show number
                    </button>
                  )}
                </div>
        ...
        <Modal isOpen={isOpen} onClose={handleModalClose} size="full">
          ...
          <ModalContent>
            <div className="absolute top-4 right-4 flex gap-3 z-50">
              <button onClick={handleZoomIn} className="p-2 rounded-full bg-black/50 hover:bg-black/70 transition-colors text-white" title="Zoom In">
                <ZoomIn size={24} />
              </button>
              <button onClick={handleZoomOut} className="p-2 rounded-full bg-black/50 hover:bg-black/70 transition-colors text-white" title="Zoom Out">
                <ZoomOut size={24} />
              </button>
              <button onClick={handleModalClose} className="p-2 rounded-full bg-black/50 hover:bg-black/70 transition-colors text-white" title="Close">
                <X size={24} />
              </button>
            </div>
            <ModalBody className="flex justify-center items-center bg-black/90">
              ...
              ${currentImageIndex === index ? "bg-blue-500 scale-125" : "bg-gray-300 hover:bg-gray-400"}
              shadow-md`}
                    aria-label={`Go to image ${index + 1}`}
                  />
                ))}
              ...
              <div className="absolute bottom-8 right-8 flex gap-4">
                ...
                <div className="bg-black/50 px-3 py-1 rounded-full text-white text-sm">{(zoomLevel * 100).toFixed(0)}%</div>
              </div>
            </ModalBody>
          </ModalContent>
        </Modal>
        ...
        <div className="mt-8">
          <SimilarAds adId={adId} adCategoryId={adCategoryId} />
        </div>

        {adData.adShowroom && (
          <div className="mt-2">
            <NearbyShowroomAds adShowroomId={adData.adShowroom.id} />
          </div>
        )}

        {adData.adShowroom && (
          <div className="mt-2">
            <FindOtherShowrooms adId={adId} adCategoryId={adCategoryId} adShowroomId={adData.adShowroom.id} />
          </div>
        )}

        <PriceAdjuster initialPrice={adData.price} onSubmit={handleOfferSubmit} isOpen={isPriceAdjusterOpen} onClose={() => setIsPriceAdjusterOpen(false)} />

        <LoginModal isOpen={isLoginModalOpen} onClose={() => setIsLoginModalOpen(false)} />

        <ChatWarningModal phoneNumber={adData.adSeller?.phone} isOpen={isChatWarningOpen} onClose={handleWarningClose} />
